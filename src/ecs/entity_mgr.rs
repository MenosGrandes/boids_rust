...
use super::{entity::{Entity, MAX_ENTITIES}, signature::Signature};
...
impl EntityManager
{
    pub fn new() -> Self
    {
        let e : VecDeque<Entity> = (0..=MAX_ENTITIES).into_iter().map(|id| id as Entity).collect();
        let signatures = [(); MAX_ENTITIES as usize].map(|_|Signature::empty());
        EntityManager{living_count : usize::MIN, avail_entities : e, signatures }
    }
